...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Hand : MonoBehaviour
    {
        ...
        public void AttachObject(GameObject objectToAttach, GrabTypes grabbedWithType, AttachmentFlags flags = defaultAttachmentFlags, Transform attachmentOffset = null)
        {
            ...
            {
                if (attachedObject.interactable.attachEaseIn && this.otherHand.currentAttachedObject == objectToAttach) // Note: modified to not ease if already attached to other hand?
                {
                ...
                if (attachedObject.interactable.handAnimationOnPickup != 0)
                    SetAnimationState(attachedObject.interactable.handAnimationOnPickup); //chpose?

                if (attachedObject.interactable.setRangeOfMotionOnPickup != SkeletalMotionRangeChange.None) //chpose?
            ...
            if (attachedObject.HasAttachFlag(AttachmentFlags.ParentToHand)) //?Is this being called for attachmentFlags=0?
            {
            ...
            {
                if (attachedObject.interactable != null && attachedObject.interactable.skeletonPoser != null && HasSkeleton()) //? is this running correctly?
                {
